...
jQuery(function ($) {
    ...
    $.getJSON('php/get-tweets.php', function (feeds) {
        ...
        for (var i = 0; i < feeds.length; i++) {
            ...
            if (
                (showretweets == true || (isaretweet == false && showretweets == false)) &&
                ...
                if (feeds[i].text.length > 1 && displayCounter <= displaylimit) {
                    ...
                    feedHTML +=
                        ...
                        profileimage +
                        '"images/twitter-feed-icon.png" width="40" height="40" alt="Avatar" /></a>';
                    feedHTML += '</figure>';
                    feedHTML += '<div class="twitter-feed__info">';
                    feedHTML += '<h5 class="twitter-feed__name">' + tweetscreenname + '</h5>';
                    feedHTML +=
                        '<h6 class="twitter-feed__username"><a href="https://twitter.com/' +
                        tweetusername +
                        '" target="_blank">@' +
                        tweetusername +
                        '</a></h6>';
                    ...
                    if (showtweetactions == true) {
                        ...
                        feedHTML +=
                            ...
                            tweetid +
                            '" class="twitter-feed__reply"></a>';
                        feedHTML +=
                            '<a href="https://twitter.com/intent/retweet?tweet_id=' +
                            tweetid +
                            '" class="twitter-feed__retweet"></a>';
                        feedHTML +=
                            '<a href="https://twitter.com/intent/favorite?tweet_id=' +
                            tweetid +
                            '" class="twitter-feed__favorite"></a>';
                        feedHTML += '</div>';
                    }
    ...
    function addlinks(data) {
        ...
        data = data.replace(/\B@([_a-z0-9]+)/gi, function (reply) {
            return (
